...
```{r corr_ca9_tabla_fig, echo=FALSE, message=FALSE, fig.width=5, fig.height=10, fig.cap="Genes con correlación significativa con CA9 (|r| ≥ 0.2, p < 0.05)"}
suppressPackageStartupMessages({
  library(dplyr)
  library(knitr)
  library(gridExtra)
...
corr_list <- lapply(genes_of_interest, function(g){
  ...
  idx <- which(!is.na(ca9_series) & !is.na(gene_vals))
  if (length(idx) >= 3) {
...
```{r corr_hif1a_tabla_fig, echo=FALSE, message=FALSE, fig.width=5, fig.height=10, fig.cap="Genes con correlación significativa con HIF1A (|r| ≥ 0.2, p < 0.05)"}
suppressPackageStartupMessages({
  library(dplyr)
  library(knitr)
  library(gridExtra)
...
corr_list <- lapply(genes_of_interest, function(g){
  ...
  idx <- which(!is.na(hif1a_series) & !is.na(gene_vals))
  if (length(idx) >= 3) {
...
```{r corr_ldha_tabla_fig, echo=FALSE, message=FALSE, fig.width=5, fig.height=10, fig.cap="Genes con correlación significativa con LDHA (|r| ≥ 0.2, p < 0.05)"}
suppressPackageStartupMessages({
  library(dplyr)
  library(knitr)
  library(gridExtra)
