...
---
title: "Análisis de expresión por cuartiles de PDL1 con infiltrado inmune / genes metabólicos (TCGA, tejido tumoral)"
output:
  html_document:
    toc: true
    toc_float: true
...
```{r, echo=FALSE, message = FALSE}
genes_of_interest <- c("HIF1A", "SLC2A1", "SLC16A3", "LDHA", "CA9", "CD80", "CD86", "HLA-DRA", "IL12B", "NOS2", "TNF", "IL1B", "CXCL9", "CXCL10", "CD163", "MRC1", "IL10", "TGFB1", "MMP9", "VEGFA", "CHI3L1", "CD14", "CD33", "ITGAM", "ARG1", "S100A8", "S100A9", "STAT3", "FUT4", "CYBB", "NCAM1","FCGR3A", "KLRK1", "NCR1", "NCR3", "PRF1", "GZMB", "GZMA", "IFNG", "KIR2DL1", "KIR3DL1", "CD8A", "CD8B", "TBX21", "CXCR3", "CD4", "IL2", "STAT1", "RORC", "IL17A", "IL17F", "IL23R", "FOXP3", "IL2RA", "CTLA4", "IKZF2", "IL7R")
...
plot_gene_box <- function(gene_name, df_in, patients_low, patients_high, cd274_series) {
  ...
  common_idx <- which(names(cd274_series) %in% extremos &
                    !is.na(cd274_series) & !is.na(gene_vals))
...
```{r corr_cd274_tabla_fig, echo=FALSE, message=FALSE, fig.width=5, fig.height=10, fig.cap="Genes con correlación significativa con CD274 (|r| ≥ 0.2, p < 0.05)"}
suppressPackageStartupMessages({
  library(dplyr)
  library(knitr)
  library(gridExtra)
...
corr_list <- lapply(genes_of_interest, function(g){
  ...
  gene_vals <- suppressWarnings(as.numeric(unlist(res$row[, names(cd274_series)], use.names = FALSE)))
  names(gene_vals) <- names(cd274_series)

  extremos <- c(patients_low, patients_high)
  idx <- which(names(cd274_series) %in% extremos & !is.na(cd274_series) & !is.na(gene_vals))

  if (length(idx) >= 3) {
    ct <- suppressWarnings(cor.test(cd274_series[idx], gene_vals[idx], method = "pearson"))
...
if (nrow(tab_out) == 0) {
  cat("**No se encontraron genes con |r| ≥ 0.2 y p < 0.05 respecto de CD274.**")
} else {
  print(
    DT::datatable(
      tab_out,
      ...
      caption = htmltools::tags$caption(
        ...
        'Genes con correlación significativa con CD274 (|r| ≥ 0.2, p < 0.05)'
      )
    )
  )
}
...
cat("\n\nLos genes que se correlacionan positivamente con CD274 son: ",
    if (length(pos_genes)) paste(pos_genes, collapse = ", ") else "ninguno", ".\n", sep = "")

cat("\nLos genes que se correlacionan negativamente con CD274 son: ",
