...
---
title: "Análisis de expresión por cuartiles de PDL1 con infiltrado inmune (TCGA, tejido tumoral)"
output:
  html_document:
    toc: true
    toc_float: true
...
Luego tomamos la expresión del gen **CD24**, calculamos **cuartiles** según la definición:
...
- **CD24 low (Q1)**: valores ≤ Q1
- **CD24 high (Q3)**: valores ≥ Q3
...
# CD274
```{r, echo=FALSE, message = FALSE, results='hide'}
# Tomar la(s) fila(s) con gene-name == "CD24"
ca9_rows <- dplyr::filter(df_tumor, `gene-name` == "CD274")

if (nrow(ca9_rows) == 0) {
  stop("No se encontró el gen CD24 en la columna 'gene-name'.")
}

if (nrow(ca9_rows) > 1) {
  message(sprintf("Aviso: se encontraron %d filas para CD24; se usará la primera (gene-id=%s).",
                  nrow(ca9_rows), ca9_rows$`gene-id`[1]))
}

ca9_row <- ca9_rows[1, ]

# Extraer valores numéricos de CD24 en las muestras tumorales
...
## Definición de grupos (CD24 low / high)
...
ca9_summary <- tibble::tibble(
  Grupo = c("CD24 low (Q1)", "CD24 high (Q3)"),
...
knitr::kable(ca9_summary, caption = "Resumen de grupos CD24 (tumor YY 01–09)")
```

## Comparación de genes de interés

```{r, echo=FALSE, message = FALSE}
genes_of_interest <- c("CD80", "CD86", "HLA-DRA", "IL12B", "NOS2", "TNF", "IL1B", "CXCL9", "CXCL10", "CD163", "MRC1", "IL10", "TGFB1", "MMP9", "VEGFA", "CHI3L1", "CD14", "CD33", "ITGAM", "IL10", "ARG1", "NOS2", "S100A8", "S100A9", "STAT3", "FUT4", "CD33", "S100A8", "S100A9", "ARG1", "CYBB", "NCAM1", "FCGR3A", "KLRK1", "NCR1", "NCR3", "PRF1", "GZMB", "GZMA", "IFNG", "KIR2DL1", "KIR3DL1", "CD8A", "CD8B", "PRF1", "GZMB", "IFNG", "TBX21", "CXCR3", "CD4", "TBX21", "IFNG", "IL2", "STAT1" ,"RORC", "IL17A", "IL17F", "IL23R", "FOXP3", "IL2RA", "CTLA4", "IKZF2", "TGFB1", "IL10",  "IL7R")
...
knitr::kable(summary_df,
             caption = "Diferencias de expresión entre CD24 low (Q1) y CD24 high (Q3)")

```


```{r, echo=FALSE, message = FALSE, results='hide'}
low_file <- "CD24_low_patients.txt"
high_file <- "CD24_high_patients.txt"
map_file <- "CD24_patient_groups.csv"
...
map_df <- tibble::tibble(
  ...
  group = c(rep("CD24 low", length(patients_low)), rep("CD24 high", length(patients_high)))
)
...
plot_gene_box <- function(gene_name, df_in, patients_low, patients_high, ca9_series) {
  ...
  # Expresión del gen de interés para las mismas muestras en las que tenemos CD24
  gene_vals <- suppressWarnings(as.numeric(unlist(row[, names(ca9_series)], use.names = FALSE)))
  names(gene_vals) <- names(ca9_series)
  
  # Correlación Pearson con CD24 (solo en muestras con datos no-NA en ambos)
  common_idx <- which(!is.na(ca9_series) & !is.na(gene_vals))
  ...
  d <- dplyr::bind_rows(
    tibble::tibble(expr = vals_low,  group = "CD24 low"),
    tibble::tibble(expr = vals_high, group = "CD24 high")
  ...
  ggplot(d, aes(x = group, y = expr, color = group)) +
    ...
    scale_color_manual(values = c("CD24 low" = "blue", "CD24 high" = "red")) +
    labs(
