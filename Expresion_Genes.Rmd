...
if (file.exists(path_local)) {
  ...
  df <- read.delim(
    "https://raw.githubusercontent.com/Lamartaav/TESIS/refs/heads/main/genes_subset_sin_dup.tsv", #tabla de pacientes con estadio IV, donde se muestra la expresión de los genes de interés. 37 columnas (3 de genes, 31 pacientes de tejido tumoral, 3 pacientes de tejido normal)
    sep = "\t", check.names = FALSE, stringsAsFactors = FALSE
  )
}

# Tomar solo columnas tumorales 01A (si no hay, usar todas), sacamos las 11A
...
# --- Función para graficar un gen (puntos + línea y etiqueta de mediana) ---
                      ...
                      col_puntos  = "#6BAED6")  # celeste
{
  ...
  med_val <- stats::median(dat$expression, na.rm = TRUE)
  lab_df  <- data.frame(gene = canon, med = med_val)

  g <- ggplot(dat, aes(x = gene, y = expression)) +
    geom_boxplot(width = 0.5,
                 fill = "white", colour = "black",
                 outlier.shape = 21, outlier.size = 2.2, outlier.fill = "white") +
    # línea de mediana resaltada
    stat_summary(fun = median, geom = "crossbar", width = 0.5,
                 colour = col_mediana, linewidth = 1.4) +
    # puntos (antes eran cruces shape = 4)
    geom_point(position = position_jitter(width = 0.08, height = 0),
               shape = 16, size = 2.2, alpha = 0.85, colour = col_puntos) +
    # etiqueta numérica de la mediana
    geom_text(data = lab_df,
              aes(x = gene, y = med, label = sprintf("%.2f", med)),
              vjust = -0.8, fontface = "bold", colour = col_mediana, size = 4) +
    labs(x = NULL,
    ...
    # un poco más de espacio arriba para que no se corte la etiqueta
...
```{r render_grid, fig.width=13, fig.height=12, results='asis'}
...
render_grid <- function(titulo, genes, ncol = 3) {
  ...
  tryCatch({
    p <- plot_gene(g)
    # achicar títulos/ejes y AUMENTAR MARGEN alrededor del plot
    p + theme(
      plot.title   = element_text(size = 14, face = "bold"),
      axis.title.y = element_text(size = 12),
      axis.text.y  = element_text(size = 10),
      plot.margin  = margin(10, 24, 10, 24)  # ↑ margen: top, right, bottom, left (pt)
    )
  }, error = function(e) {
    ggplot() +
      annotate("text", x = 0, y = 0,
               label = paste("Error con", g, ":", e$message)) +
      theme_void()
  })
})
