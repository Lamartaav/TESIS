...
dat_luad <- expr_ca9 %>%
  ...
  # Filtrar proyecto LUAD explícito en clínica
...
# Tamaños por grupo
table(dat_luad$CAIX_group)

```

``` {r, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# Aseguramos que CA9 es numérico
dat_luad$CA9 <- suppressWarnings(as.numeric(dat_luad$CA9))

# 1) Grupos por MEDIANA
med <- median(dat_luad$CA9, na.rm = TRUE)
dat_med <- dat_luad %>%
...
p_med <- survminer::ggsurvplot(
  ...
  xlab = "Time (months)", ylab = "Overall survival probability (TCGA-LUAD)",
