---
title: "Kaplan–Meier para CAIX (CA9) en TCGA-LUSC"
...
## 3. Preparar tabla clínica lusc (limpia, sin duplicados)
```{r}
...
data_merged <- expr_ca9 %>%
  inner_join(clinical_LUSC, by = c("submitter_id" = "cases.submitter_id")) %>%
...
ggsurvplot(
  ...
  title = "CAIX and Overall Survival (TCGA-LUSC) – Cuartiles",
  xlab = "Time (months)",
...
ggsurvplot(
  ...
  legend.title = "CAIX (CA9)",
  legend.labs = c("Low", "High"),
  title = "CAIX and Overall Survival (TCGA-LUSC) – Mediana",
  xlab = "Time (months)",
  ylab = "Overall survival probability",
  risk.table.height = 0.25,
  palette = c("#4A90E2", "#E91E63")
